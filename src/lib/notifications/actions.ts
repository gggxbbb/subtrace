...
import { createChannel, deleteChannel, setChannelEnabled, testChannel } from "./service";
...
export async function createChannelAction(formData: FormData) {
  ...
  if (kind === "WEBHOOK") {
    ...
    if (!host || !Number.isInteger(port) || !from || !to) redirect(`${PATH}?error=1`);
    config = {
      host,
      port,
