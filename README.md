...
控制台顶部 banner 可升级为 AI 摘要：LLM 从引擎已算出的信号（到期、盈亏、浪费、支出水平、回本进度）中选题成文。摘要只转述引擎数字，不生产新事实；LLM 不可用时回退模板文案，用户无感（见 ADR-0016）。

接入方式为实例级环境变量三件套，任何 **OpenAI 兼容 chat completions 端点**均可（DeepSeek、OpenRouter、自托管 vLLM 等）：

| 变量 | 说明 | 示例 |
|---|---|---|
| `LLM_BASE_URL` | chat completions 端点基础地址 | `https://api.deepseek.com/v1` |
| `LLM_API_KEY` | 该端点的 API Key | `sk-…` |
| `LLM_MODEL` | 模型名 | `deepseek-chat` |

**配置即开启**：三件套配齐功能才存在；**启用即同意把订阅名称、金额、用量统计外发给该端点**（不做脱敏，介意勿配）。不配置则功能不存在——代码路径不激活、banner 维持模板态、全站零差异、不发出任何 LLM 请求。
